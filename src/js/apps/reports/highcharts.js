...
function getTooltipTitle(key, formatter) {
  return 'function' === typeof formatter ? formatter(key) : key;
}
...
function getTooltipFormatter(data) {
  ...
  return function() {
    const $table = $(`<table class="${data.className ? data.className : ''}">`),
      $header = addTooltipHeader(getTooltipTitle(this.key, data.titleFormatter), 2),
      $body = $('<tbody>');
...
function getSharedTooltipFormatter(data) {
  ...
  return function() {
    const $table = $('<table>'),
      $header = addTooltipHeader(getTooltipTitle(this.points[0].key, data.titleFormatter), 2),
      $body = $('<tbody>');
